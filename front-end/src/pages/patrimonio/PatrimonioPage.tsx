...
type PatrimonioTipo = "" | "Imóvel" | "Veículo" | "Investimento" | "Equipamento" | "Outros";
type NavItem = "Home" | "Despesas";

const TIPOS: PatrimonioTipo[] = ["Imóvel", "Veículo", "Investimento", "Equipamento", "Outros"];
...
const BoxIcon = () => (
  <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={COLORS.primary} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
...
const TextInput = ({ value, onChange, placeholder }: TextInputProps) => (
  <input
    ...
    onBlur={(e) => { e.currentTarget.style.borderColor = COLORS.primaryBorder; }}
  />
);
...
const SubmitButton = ({ isValid, submitted, onClick }: SubmitButtonProps) => (
  <button
    ...
    onMouseEnter={(e) => {
      if (isValid) e.currentTarget.style.transform = "translateY(-1px)";
    }}
    onMouseLeave={(e) => {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="w-full px-4 py-3.5 rounded-xl flex items-center justify-between cursor-pointer text-sm transition-colors duration-200 outline-none"
        style={{
          background: COLORS.primaryLight,
        ...
        >
          {TIPOS.map((tipo) => (
            <button
              ...
              className="w-full px-4 py-3 text-left text-sm cursor-pointer transition-colors duration-150"
              style={
                value === tipo
              ...
              onMouseEnter={(e) => {
                if (value !== tipo) e.currentTarget.style.background = COLORS.primaryLight;
              }}
              onMouseLeave={(e) => {
                if (value !== tipo) e.currentTarget.style.background = "transparent";
              }}
            >
              {tipo}
            </button>
...
const Navbar = () => {
  ...
  const activeNav = (Object.entries(NAV_ROUTES) as [NavItem, string][])
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-1 rounded-xl p-1" style={{ background: "rgba(255,255,255,0.12)" }}>
...
const BackButton = ({ onClick }: BackButtonProps) => (
  <button
    type="button"
    onClick={onClick}
    className="absolute top-8 left-8 w-10 h-10 rounded-xl border bg-white flex items-center justify-center cursor-pointer transition-shadow duration-200 hover:shadow-md"
    style={{ borderColor: COLORS.primaryBorder, boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}
  >
    <ArrowLeftIcon />
  </button>
);
...
const CardHeader = () => (
  <div className="text-center mb-7">
    ...
    <div
      className="w-8 h-0.5 rounded-full mx-auto mt-3"
      style={{ background: COLORS.primary }}
    />
  </div>
);
...
const PatrimonioForm = ({
  form, submitted, isValid,
  onTipoChange, onNomeChange, onValorChange, onSubmit,
}: PatrimonioFormProps) => (
...
export default function CadastrarPatrimonio() {
  ...
  const isValid = form.tipo !== "" && form.nome.trim() !== "" && form.valor !== "";

  return (
    <div className="min-h-screen flex flex-col font-sans" style={{ background: COLORS.primaryLight }}>
      <Navbar />
